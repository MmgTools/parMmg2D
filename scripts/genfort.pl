...
#    >     -s <size>   Defines PMMG2D_INT  kind (4 or 8 usually)
#
#   Authors:
...
my $int      = 0;
my $fichier;
#my $format = "PMMG2D_INTEGER, PARAMETER :: %-30s = %d";
my $format = "#define %-30s %d";
my $formatbyval = "#define %-30s \%val(%d)";
my $definebyval = "#define PMMG2D_ARG_%-30s \%val(%d)\n";
my %opts;
...
sub Usage {
    ...
    print "  converts the file libparmmg*.h into a fortran header.\n";
...
sub printTab # ($chaine, $tabcount, $comm)
{
    ...
    if ($comm == 0)
    {
        if ($int != 0)
        {
            $chaine =~ s/PMMG2D_INTEGER,/INTEGER(KIND=$int),/g;
        }
        else
        {
            $chaine =~ s/PMMG2D_INTEGER,/INTEGER,/g;
        }
        if ($real != 0)
        {
    ...
    foreach my $line ( <APIc> )
    {
        if ($startcom == 0)
        {
            if ($startenum == 0)
            {
                ...
                elsif ($line =~ /\#define PMMG2D_ARG_(\w*)\s+(.*)/)
                {
        ...
        else
        {
            ...
            elsif($line =~ /(.*)/)
            {
                ...
                if ($line =~ /ParMmg2's constants/)
                {
                    ...
                    $chaine2 .= "!\n";
                    $chaine2 .= "!   Contains:\n";
                    $chaine2 .= "!     PMMG2D_DATA_PTR_T - Kind to use for POINTERS\n";
                   # $chaine2 .= "!     PMMG2D_REAL_KIND  - Kind to use for REAL\n";
                   # $chaine2 .= "!     PMMG2D_INT_KIND   - Kind to use for INT\n";
                    $chaine2 .= "!\n";
                    if ($real != 0)
                    {
                        $chaine2 .= "INTEGER, PARAMETER :: PMMG2D_REAL_KIND                = $real\n";
                    }
                    if ($int != 0)
                    {
                        $chaine2 .= "INTEGER, PARAMETER :: PMMG2D_INT_KIND                = $int\n";
                    }
                    $tabcount --;
