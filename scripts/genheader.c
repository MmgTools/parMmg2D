...
{
  char * header_f          = NULL;
  char * libparmmg2d_h       = NULL;
  char * libparmmg2d_include = NULL;
  char * genfort           = NULL;
  char * cmd               = NULL;
  FILE * file              = NULL;
  ...
  if (argc != 5)
    {
      fprintf(stderr, "usage : %s headerNameFortran.h headerNameC.h"
              " libparmmg2dtypesf_include_dir genfort.pl\n",argv[0]);
      return EXIT_FAILURE;
    }

  header_f            = argv[1];
  libparmmg2d_h       = argv[2];
  libparmmg2d_include = argv[3];
  genfort             = argv[4];
  ...
  fprintf(file,"! **\n");
  fprintf(file,"! ** This scripts recopy the macros of the libparmmg2d*.h file\n");
  ...
  if(strstr(libparmmg2d_h,"libparmmg2dtypes.h")==NULL) {
    fprintf(file,"#include \"mpif.h\"\n\n");
    fprintf(file,"#include \"%s/libparmmg2dtypesf.h\"\n\n",libparmmg2d_include);
  }
  else {
    fprintf(file,"#include \"mmg/mmg2d/libmmg2dtypesf.h\"\n\n");
  }
  fclose(file);

  /* Generate Fortran header */
  if (NULL == (cmd = (char*)malloc((strlen(genfort)+
                                    strlen(libparmmg2d_h)+
                                    strlen(header_f)+128)*sizeof(char))))
    return EXIT_FAILURE;
  sprintf(cmd, "perl %s -f %s >> %s;",
          genfort, libparmmg2d_h, header_f);
  fprintf(stdout, "%s\n", cmd);
  if (-1 == system(cmd))
    return EXIT_FAILURE;
  free(cmd);
